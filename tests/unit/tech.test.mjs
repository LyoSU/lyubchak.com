...
const chaos = html.match(/<div class="chaos"[\s\S]*?<\/div>/)[0];
const used = [...chaos.matchAll(/--m:url\(\/images\/tech\/([a-z0-9]+)\.svg\)/g)].map(m => m[1]);
...
test('icon names are available to screen readers and crawlers', () => {
  const sr = [...html.matchAll(/<p class="sr"[^>]*>([^<]+)<\/p>/g)].map(m => m[1]).find(t => t.includes('TypeScript')) || '';
